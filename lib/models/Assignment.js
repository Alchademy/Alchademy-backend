...
module.exports = class Assignment {
  ...
  static async getAllAssignmentsWithGradesBySyllabusIdAndUserId(
    ...
    const { rows } = await pool.query(
      `
      SELECT a.id, a.title, a.description, a.syllabus_id, a.due_date, a.total_points, a.status_id, s.id as submission_id, s.created_on, s.text, s.status_id as submission_status_id, s.assignment_id, s.user_id, s.grade
      FROM assignments a
