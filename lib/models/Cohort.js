...
module.exports = class Cohort {
  ...
  static async getCohortByIdAndUser(id, user_id) {
    const { rows } = await pool.query(
      `SELECT *
      FROM cohorts c
      join user_to_cohort utc on c.id = utc.cohort_id
      where c.id = $1
      and utc.user_id = $2;`,
      [id, user_id]
    );
  ...
  //needed for update, but should only be used in TA and Teach authorized calls
  ...
  static async getCohortByUserId(id) {
    const { rows } = await pool.query(
      `SELECT *
    FROM cohorts c
    join user_to_cohort utc on c.id = utc.cohort_id
  ...
  static async delete(id) {
    ...
    await pool.query('delete from cohort_to_syllabus where cohort_id=$1;', [
      id,
    ]);
    const { rows } = await pool.query(
