...
module.exports = Router()
  ...
  .get('/callback', async (req, res, next) => {
    try {
      ...
      //     const arr = [];
      //     for (const team of TATeams) {
      //       const TAMembers = await getTAMembers(team.id);
      //       TAMembers.forEach((item) => arr.push(item));
      //     }
      //     const TA = arr.filter((ta) => ta.login === githubProfile.login);
      //     if (TA.length > 0) {
      //       role = 2;
      //     }
      //   }
      ...
      //   // if the user was found as active in the cohortCheck array, push the name of that github team to the cohort array
      //   const cohortMembers = await getCohortMembers(studentTeams[i].id);
      //   const cohortian = cohortMembers.filter(person => person.login === githubProfile.login);
      //   for (const member of cohortMembers) {
      //     if (member.login === githubProfile.login) {
      //       cohorts.push(studentTeams[i].name);
      //       break;
      //     }
      //   }
