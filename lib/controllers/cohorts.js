...
module.exports = Router()
  .get('/:id', authenticate, authorize, async (req, res, next) => {
    try {
      const singleCohort = await Cohort.getCohortByIdAndUser(req.params.id, req.user.id);
      if (!singleCohort) throw new Error('You do not have access to this record!');
  ...
  .put('/:id', authenticate, authorizeTeacher, async (req, res, next) => {
    try {
      const updatedCohort = await Cohort.updateById(req.params.id, req.body, req.user.id);
      if (!updatedCohort) throw new Error('You do not have access to this record!');
